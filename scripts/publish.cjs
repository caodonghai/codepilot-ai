...
async function selectRelease() {
  ...
  const answer = await ask('请选择升级类型 [1=patch, 2=minor, 3=major]：');
  const selected = { 1: 'patch', 2: 'minor', 3: 'major' }[answer] || answer;
  if (!allowedReleases.has(selected)) throw new Error(`无效的版本类型：${answer}`);
  return selected;
}
