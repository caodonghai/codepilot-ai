...
`npm run publish` 会先构建并检查 npm 打包内容，然后通过上下方向键选择 `patch`、`minor` 或 `major`（默认选择 `patch`，回车确认），自动更新版本、创建 `chore(release): v<version>` 提交和 Git Tag、推送到 `origin`，最后发布到 npm。非交互环境可使用 `npm run publish -- patch --yes`。发布前必须保证 Git 工作区干净并已登录 npm。
