...
exports.collectSkillSummary = exports.collectFlowSummary = exports.collectCoreSummary = exports.normalizeTools = exports.applyToolSkip = exports.listTargetFiles = exports.templateChangeFile = exports.seedProjectTemplates = exports.writeFileIfMissingFromTemplate = exports.findTemplateRoot = exports.setupPackageScript = void 0;
...
exports.setupPackageScript = setupPackageScript;
...
exports.findTemplateRoot = findTemplateRoot;
...
exports.writeFileIfMissingFromTemplate = writeFileIfMissingFromTemplate;
...
exports.seedProjectTemplates = seedProjectTemplates;
function renderTemplate(content, data) {
    return content.replace(/\{\{(\w+)\}\}/g, (_, key) => { var _a; return (_a = data[key]) !== null && _a !== void 0 ? _a : key; });
}
function templateChangeFile(change, kind, type = 'default') {
    const templateRoot = findTemplateRoot();
    if (templateRoot) {
        const templatePath = path_1.default.join(templateRoot, 'changes', type, kind);
        if (fs_1.default.existsSync(templatePath)) {
            const content = fs_1.default.readFileSync(templatePath, 'utf8');
            return renderTemplate(content, { change, type });
        }
        const fallbackPath = path_1.default.join(templateRoot, 'changes', 'default', kind);
        if (fs_1.default.existsSync(fallbackPath)) {
            const content = fs_1.default.readFileSync(fallbackPath, 'utf8');
            return renderTemplate(content, { change, type });
        }
    }
    if (kind === 'proposal.md') {
...
exports.templateChangeFile = templateChangeFile;
...
exports.listTargetFiles = listTargetFiles;
function applyToolSkip(tools, skipValue) {
    if (!skipValue)
        return tools;
    const skipped = skipValue.split(',').map((item) => item.trim()).filter((tool) => utils_1.defaultTools.includes(tool));
    return tools.filter((tool) => !skipped.includes(tool));
}
exports.applyToolSkip = applyToolSkip;
...
exports.collectCoreSummary = collectCoreSummary;
...
exports.collectFlowSummary = collectFlowSummary;
function collectSkillSummary() {
    return utils_1.skillFiles.map((file) => `- superpowers/skills/${file}`).join('\n');
}
exports.collectSkillSummary = collectSkillSummary;
