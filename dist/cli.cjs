...
function newCommand(changeInput, options = {}) {
    var _a, _b, _c, _d;
...
function buildKnowledgeAddCommand(record) {
    ...
    if (record.scope)
        args.push(`--scope "${record.scope}"`);
    if (record.source)
        args.push(`--source "${record.source}"`);
    if ((_a = record.keywords) === null || _a === void 0 ? void 0 : _a.length)
        args.push(`--keywords "${record.keywords.join(',')}"`);
    if ((_b = record.usedIn) === null || _b === void 0 ? void 0 : _b.length)
        args.push(`--used-in "${record.usedIn.join(',')}"`);
...
function runCommand(command, args) {
    const startedAt = Date.now();
    const result = (0, child_process_1.spawnSync)(command, args, { cwd: root, shell: true, stdio: 'inherit' });
    const exitCode = typeof result.status === 'number' ? result.status : 1;
    return {
